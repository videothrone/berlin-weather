...
export const makeApiCall = async (): Promise<WeatherData> => {
  ...
  try {
    const response = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=Berlin,Germany&appid=${openWeatherAPIKey}&units=metric`,
    );
    const data = await response.json();

    const responseForecast = await fetch(
      `https://api.openweathermap.org/data/2.5/forecast?q=Berlin,Germany&units=metric&appid=${openWeatherAPIKey}`,
    );
